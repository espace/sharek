...
var amin2 = canvas.zeroToTenArc(160, 130, 100, 1).attr({ stroke : "#79af01", "stroke-width" : 22 });
...
this.toFront();
	...
	$('.fir').stop().animate({ opacity: 0 }, 100, function(){
			  $(this).html(title).attr('rel', text).removeAttr('title').animate({ opacity: 1 });
			  // $(this).html(text).attr('title', title).removeAttr('rel').animate({ opacity: 1 })
		  ;}); 
