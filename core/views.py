...
def top_users_map(request):
    ...
    if user == None:
        ...
        temp_users = Feedback.objects.values('user').annotate(user_count=Count('user')).order_by('-user_count').exclude(user__in=inactive_users).exclude(user=user.username)[:500]
