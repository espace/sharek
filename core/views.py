        ...
        if request.method == 'POST':
            ...
            if request.user.username != "admin":
                ...
                if UserSocialAuth.auth_provider(request.user.username) == 'twitter':
                    ...
                    link = settings.domain+"sharek/"+request.POST.get("class_slug")+"/"+request.POST.get("article_slug")+"/comment/"+str(feedback[0].id)+"/"
                    message = 'لقد شاركت في كتابة #دستور_مصر وقمت بالتعليق على '+get_object_or_404(ArticleDetails, id=request.POST.get("article")).header.name.encode('utf-8')+ str(link) +"  من الدستور"
