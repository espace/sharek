...
def top_users_map(request):
    ...
    inactive_users = User.get_inactive
    if user == None:
        temp_users = User.objects.order_by('?').exclude(user__in=inactive_users)[:2000] #Feedback.objects.values('user').annotate(user_count=Count('user')).order_by('-user_count').exclude(user__in=inactive_users)[:2000]
    else:
        temp_users = User.objects.order_by('?').exclude(user__in=inactive_users).exclude(user=user.username)[:2000] #Feedback.objects.values('user').annotate(user_count=Count('user')).order_by('-user_count').exclude(user__in=inactive_users).exclude(user=user.username)[:2000]
